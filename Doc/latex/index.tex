La classe \doxyref{R\+F\+M69}{pag.}{class_r_f_m69} permette di collegare due microcontrollori tramite moduli radio della famiglia R\+F\+M69 di Hope\+RF, e in particolare tramite il modulo R\+F\+M69\+H\+CW ({\tt http\+://www.\+hoperf.\+com/rf\+\_\+transceiver/modules/\+R\+F\+M69\+H\+C\+W.\+html}). Non ho eseguito alcun test sugli altri moduli.

Alla fine di questo testo si trova un esempio dell\textquotesingle{}utilizzo di questa classe.

\begin{DoxyParagraph}{Caratteristiche del modulo radio}
...
\begin{DoxyParagraph}{Protocollo di comunicazione}
...
Gli schemi sottostanti illustrano la trasmissione di un mesasggio. Nel primo caso si tratta di un messaaggio con richiesta di A\+CK, nel secondo no. 
\begin{DoxyCode}
mod       ?   |    tx    |            rx             |        def
fz          INVIA       ISR                         ISR
A       ------|----------|---------------------------|-------------------->
...
\begin{DoxyParagraph}{Collisioni}
...
Lo schema sottostante mostara i momenti in cui non si può o non si dovrebbe trasmettere. Il primo schema si riferisce ai messaggi con richiesat di A\+CK, il secondo a quelli senza. 
\begin{DoxyCode}
stato tx                    |*********|############ 1 ############|
A           ----------------|---------|---------------------------|---------->
|                         INVIA      ISR                         ISR
...
\item [$\ast$$\ast$$\ast$]\+: impossibile trasmettere, invia() aspetta che sia di nuovo possibile (ma al massimo 50 ms)
\item [\#\#\#]\+: la funzione invia() non dovrebbe mai essere chiamata qui\+:
\begin{DoxyEnumerate}
\item C\+H\+I\+A\+M\+A\+TA AD {\ttfamily invia()} Q\+UI --$>$ P\+R\+O\+B\+L\+E\+MA N\+EL C\+O\+D\+I\+CE D\+E\+LL\textquotesingle{}U\+T\+E\+N\+TE In teoria non bisognerebbe trasmettere (l\textquotesingle{}altra radio non è in modalità rx), ma in realtà se l\textquotesingle{}utente chiama invia() mentre la classe aspetta un ack per il messaggio precedente significa che l\textquotesingle{}utente ha rinunciato a controllare quell\textquotesingle{}ack. In tal caso invia() si comporta come se il messaggio precedente non avesse contenuto una richiesta di ack. Probabilmente questo messaggio andrà perso, ma il compito della funzione invia() non è aspettare l\textquotesingle{}ack precedente (quello è compito dell\textquotesingle{}utente, anche se lo aspettasse per un certo tempo invia() non potrebbe segnalare se è arrivato o no). La sequenza corretta sarebbe\+: invia() con richiesta ack -\/$>$ aspetta\+Ack(), che contiene un timeout -\/$>$ ack\+Ricevuto()? -\/$>$ invia() prossimo messaggio, oppure invia() -\/$>$ delay(x) -\/$>$ ack\+Ricevuto()? -\/$>$ rinuncia\+Ack() invia()
\item momento critico\+: se si chiama invia() qui ci sarà una collisione con l\textquotesingle{}invia() della radio A e entrambi i messaggi saranno persi, ma questa classe non ha modo di evitarlo. Spetta all\textquotesingle{}utente impedire queste collisioni o saperle gestire.
\item i messaggi inviati qui saranno persi. È un difetto dei messaggi senza A\+CK.
\end{DoxyEnumerate}
\end{DoxyItemize}
...
\hline
\rowcolor{\tableheadbgcolor}\textbf{ R\+F\+M69 }&\textbf{ uC  }\\\cline{1-2}
\endfirsthead
\hline
\endfoot
\hline
...
S\+CK &S\+CK \\\cline{1-2}
N\+SS &I/O $\ast$ \\\cline{1-2}
D\+I\+O0 &I\+NT $\ast$$\ast$ \\\cline{1-2}
{\itshape R\+E\+S\+ET \&} &{\itshape I/O $\ast$} \\\cline{1-2}
\end{longtabu}

\begin{DoxyItemize}
\item \&\+: Opzionale
\item $\ast$\+: O\+UT è qualsiasi pin di input/output (sarà configurato come output dalla classe)
\item $\ast$$\ast$\+: I\+NT è un pin capace di attivare un interupt del microcontrollore. Ad esempio su Atmega328p, il microcontrollore di Arduino U\+NO, si possono usare i pin 4 e 5, cioé rispettivamente 2 e 3 nell\textquotesingle{}ambiete di programmazione Arduino.
\end{DoxyItemize}

\begin{DoxyParagraph}{Struttura messaggi}
...
~\newline
{\bfseries Documentazione dettagliata dei membri della classe\+: \doxyref{R\+F\+M69}{pag.}{class_r_f_m69} }

~\newline
~\newline
\begin{DoxyParagraph}{Esempio di utilizzo}
~\newline
